...
class EWSJFScheduler(Scheduler):
    ...
    def __init__(
        ...
        elif self.external_parameters and 'min_input_length' in self.external_parameters:
            self.waiting.initialize_queues_by_length(self.external_parameters['min_input_length'],
                                                     self.external_parameters['max_input_length'],
                                                     self.step_size)
        else:
            self.waiting.initialize_queues(num_queues=20, step_size=self.step_size)
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs:
            ...
            token_budget = self._schedule_ewsjf_waiting_requests(encoder_budget, num_scheduled_tokens, req_index, req_to_new_blocks,
                                                                 scheduled_encoder_inputs, scheduled_loras, scheduled_new_reqs,
                                                                 scheduled_resumed_reqs, scheduled_timestamp,
                                                                 skipped_waiting_requests, structured_output_request_ids,
                                                                 token_budget)
    ...
    def _schedule_ewsjf_waiting_requests(self, encoder_budget, num_scheduled_tokens, req_index, req_to_new_blocks,
                                         scheduled_encoder_inputs, scheduled_loras, scheduled_new_reqs, scheduled_resumed_reqs,
                                         scheduled_timestamp, skipped_waiting_requests, structured_output_request_ids,
                                         token_budget):
        ...
        with self.lock:
            while not self.waiting.is_empty_best_queue and token_budget > 0:
                ...
                effective_lookahead_tokens = (0 if request.num_computed_tokens == 0 else
